...
export function ModuleConfigForm({ config, guild, onChange }: ModuleConfigFormProps) {
	...
	function renderRecord(path: string[], record: ConfigRecord) {
		const entries = Object.entries(record).filter(([key]) => key !== 'enabled');
		return (
	...
	return <div className="module-config-form">{renderRecord([], cloneValue(config) as ConfigRecord)}</div>;
}
